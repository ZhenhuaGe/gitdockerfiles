...
/*
  ...
  //cv::Mat  H=cvFindHomography(fr1,fr2,CV_RANSAC);
...
//detect sift KeyPoint and write them into vector<Point2f> 
  ...
  cv::Mat diff=dst-frame2.rgb;
  
  cv::imshow("diff",diff);
  */
  //compute the denseopticalflow
  cv::Mat gray1,gray2,denseflow; 
  cv::Mat motion2color;
  cv::cvtColor(frame1.rgb,gray1,CV_RGB2GRAY);
  cv::cvtColor(frame2.rgb,gray2,CV_RGB2GRAY);
  //cv::calcOpticalFlowFarneback(gray1,gray2,denseflow,0.5,3,15,3,5,1.2,0);
  
  //cv::buildOpticalFlowPyramid(gray1,,frame1.rgb.size(),4);
  
  
  cv::calcOpticalFlowSF(frame1.rgb,frame2.rgb,denseflow,5,2,4);
  //5means layers 2means averaging_block_size,10 means max flow search at each flow
  motionToColor(denseflow,motion2color);
  cv::imshow("flow",motion2color);
  
  //compute fundamental matrix
  /*
  int ptCount=(int) good_mathches.size();
  cv::Mat  p1(ptCount,2,CV_32F);
  cv::Mat  p2(ptCount,2,CV_32F);
  
  //convert keypoint to mat
  for (int i=0;i<ptCount;i++){
    p1.at<float>(i,0)=frame1.kp[good_mathches[i].queryIdx].pt.x;
    p1.at<float>(i,1)=frame1.kp[good_mathches[i].queryIdx].pt.y;
    p2.at<float>(i,0)=frame2.kp[good_mathches[i].trainIdx].pt.x;
    p2.at<float>(i,1)=frame2.kp[good_mathches[i].trainIdx].pt.y;  
  } 
  */
  cv::Mat ddst;
  cv::Mat m_fundamental=findFundamentalMat(fr1,fr2,cv::FM_RANSAC,3,0.99);
 // cv::warpPerspective(frame1.rgb,ddst,m_fundamental,frame2.rgb.size());
  //cv::imshow("fundamental",ddst);
